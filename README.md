...
![refractum icon](io.github.Labaman.refractum.svg)
...
Download the `.pkg.tar.zst` from the [latest release](https://github.com/Labaman/refractum/releases/latest) and install it:

```bash
sudo pacman -U refractum-*.pkg.tar.zst
```
...
git clone https://github.com/Labaman/refractum.git
cd refractum
makepkg -si
```
...
Refractum stores personal settings in `~/.config/refractum/settings.toml`, written
...
`/etc/refractum.toml` (requires root via pkexec). This lets an admin set
...
1. `/etc/refractum.toml` — refractum's own system-wide config (TOML)
...
The bootstrapped settings are saved immediately to `~/.config/refractum/settings.toml`,
so external config files are never read again after the first launch.

## Acknowledgements

Refractum was inspired by two existing tools:
